...
///   vk.DeviceDispatch)
///   are "plain" structs which just contain the function pointers for a
///   particular object.
/// - The Wrapper types (vk.Basewrapper, vk.InstanceWrapper, vk.DeviceWrapper)
///  contains
///   the Dispatch type, as well as Ziggified Vulkan functions - these return
///   Zig errors, etc.
/// - The Proxy types (vk.InstanceProxy, vk.DeviceProxy, vk.CommandBufferProxy,
///   vk.QueueProxy) contain a pointer to a Wrapper and also contain the 
///   object's handle.
///   Calling Ziggified functions on these types automatically passes the
///   handle as the first parameter of each function. Note that this type
///   accepts a pointer to a wrapper struct as there is a problem with
///   LLVM where embedding function pointers and object pointer in the same
///   struct leads to missed optimizations. If the wrapper
...
pub const GraphicsContext = struct {
    ...
    pub fn init(allocator: Allocator, app_name: [*:0]const u8,
        ...
        self.present_queue = Queue.init(
            self.dev, 
            candidate.queues.present_family
        );

        self.mem_props = self.instance.getPhysicalDeviceMemoryProperties(
            self.pdev
        );
    ...
    pub fn allocate(self: GraphicsContext,
                    requirements: vk.MemoryRequirements,
                    flags: vk.MemoryPropertyFlags) !vk.DeviceMemory {
        return try self.dev.allocateMemory(&.{
                .allocation_size = requirements.size,
                .memory_type_index = try self.findMemoryTypeIndex(
                    requirements.memory_type_bits, flags
                ),
            }, 
            null
        );
...
fn checkLayerSupport(vkb: *const BaseWrapper, alloc: Allocator) !bool {
    const available_layers = try vkb.enumerateInstanceLayerPropertiesAlloc(
        alloc
    );
...
fn initializeCandidate(instance: Instance,
                       candidate: DeviceCandidate) !vk.Device {
    ...
    const queue_count: u32 = if 
        (candidate.queues.graphics_family == candidate.queues.present_family)
        1
    else
        2;
...
fn debugUtilsMessengerCallback(
    ...
    _: ?*anyopaque) callconv(.c) vk.Bool32 {
    const severity_str = 
    ...
    const message: [*c]const u8 = 
        if (callback_data) |cb_data| cb_data.p_message 
        else "NO MESSAGE!";
...
fn allocateQueues(
    ...
    pdev: vk.PhysicalDevice, 
    allocator: Allocator,
    surface: vk.SurfaceKHR) !?QueueAllocation {
    const families = try instance.getPhysicalDeviceQueueFamilyPropertiesAlloc(
        pdev,
        allocator
    );
...
fn checkSurfaceSupport(
    ...
    _ = try instance.getPhysicalDeviceSurfaceFormatsKHR(
        pdev, surface, &format_count, null
    );

    var present_mode_count: u32 = undefined;
    _ = try instance.getPhysicalDeviceSurfacePresentModesKHR(
        pdev, surface, &present_mode_count, null
    );
...
fn checkExtensionSupport(
    ...
    const propsv = try instance.enumerateDeviceExtensionPropertiesAlloc(
        pdev, null, allocator
    );
